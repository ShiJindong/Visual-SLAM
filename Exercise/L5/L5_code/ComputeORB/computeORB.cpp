...
int main(int argc, char **argv) {
  ...
  evaluate_and_call([&]() { computeAngle(first_image, keypoints); },
                    "compute angle", 1);

  // test multi thread version
  evaluate_and_call([&]() { computeAngleMT(first_image, keypoints); },
                    "compute angle mt", 1);
...
void computeAngleMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
  ...
  int half_patch_size = 8;
  std::for_each(std::execution::par_unseq, keypoints.begin(), keypoints.end(), [&half_patch_size, &image](auto &kp) {
        ...
        for(int i=-half_patch_size; i<half_patch_size; i++)                 //行的递增
            for(int j=-half_patch_size; j<half_patch_size; j++) {           //列的递增
                int v = y + i; 
                int u = x + j;
                m01 += i * (float)image.at<uchar>(v, u);                   
                m10 += j * (float)image.at<uchar>(v, u); 
            }
        // 计算关键点的旋转角
  ...
  for (auto &kp : keypoints) {
    ...
    for (int i = 0; i < 256; i++) {
      // START YOUR CODE HERE (~7 lines)
        ...
        // 对p, q做边界检查，如果该描述子附近取的某个任意点p, q跑出图像外，就设这个描述子为空，并跳出当前for循环，计算下一个关键点
        if(p.x < 0 || p.x > (image.cols-1) || p.y < 0 || p.y > (image.rows-1) || q.x < 0 || q.x > (image.cols-1) || q.y < 0 || q.y > (image.rows-1) ){
            d.clear();                                           // d 的类型为 vector<bool>, 对vector进行清零
            break;
        }
...
void computeORBDescMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints,
    ...
    std::for_each(std::execution::par_unseq, keypoints.begin(), keypoints.end(), [&half_patch_size, &image, &desc](auto &kp) {
        DescType d(256, false);                                      // 初始化256个初始值为false的元素
        for (int i = 0; i < 256; i++) {
            // 取得 p 和 q相对于关键点的相对坐标
            int delta_up = ORB_pattern[4*i];
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2,
  ...
  for(int i = 0; i < desc1.size(); i++){
      ...
      int d2_match_index = -1;      // d2_match_index 为 d1 在 desc2 中匹配的 特征点的序号
      ...
      if(d_min <= d_max){
          ...
          // DMath定义: CV_WRAP DMatch(int _queryIdx, int _trainIdx, float _distance);
      }
  }
  // END YOUR CODE HERE
